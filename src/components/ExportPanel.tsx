...
export default function ExportPanel({ pages, pageSize, rows, cols, wallDimensions, scaleFactor = 1 }: ExportPanelProps) {
      ...
      <div className="p-4 bg-gray-800/50 rounded-xl border border-gray-700">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="bg-gray-900/50 rounded-lg p-3">
            ...
            <p className="text-xl font-bold text-white">{cols}×{rows}</p>
          </div>
          <div className="bg-gray-900/50 rounded-lg p-3 col-span-2">
            <p className="text-gray-500 text-xs">Page Size</p>
            <p className="text-lg font-semibold text-white">{pageSize.name} ({pageSize.width}×{pageSize.height}mm)</p>
          </div>
          <div className="bg-gray-900/50 rounded-lg p-3 col-span-2">
            <p className="text-gray-500 text-xs">Joined Image Size</p>
            <p className="text-lg font-semibold text-white">{joinedDimensions.width}×{joinedDimensions.height}mm</p>
          </div>
          {scaleFactor !== 1 && (
            <div className="bg-violet-900/30 rounded-lg p-3 col-span-2 border border-violet-700/50">
              ...
              <p className="text-lg font-semibold text-violet-300">{Math.round(scaleFactor * 100)}% of original</p>
            </div>
          )}
        </div>
      </div>
        ...
        >
          <h3 className="text-sm font-medium text-gray-300 mb-2 flex items-center gap-2">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19V6l-2 2m0 0l7-7 7 7" />
            </svg>
            Wall Fit Status
          </h3>
          <div className="space-y-2 text-sm">
            <div className="flex items-center justify-between">
              ...
              <span className={`font-semibold ${wallFitInfo.fits ? 'text-emerald-300' : 'text-red-300'}`}>
